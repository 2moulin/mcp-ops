...
import { getBalance, listCharges, listPayouts, listTransfers } from './money.js';
import { getEvent, listEvents, listWebhookEndpoints } from './webhooks.js';
import { diagnoseAccount, diagnoseTransfer } from './diagnose.js';
...
export function stripeProvider(stripe: StripeReader, mode: 'live' | 'test' | 'unknown'): Provider {
  ...
  return {
    ...
    register(server, guard: Guard) {
      ...
      server.registerTool('stripe_list_events', {
        ...
        inputSchema: { type: z.string().optional(), only_undelivered: z.boolean().optional(), limit },
      ...
      server.registerTool('stripe_get_event', {
        title: 'Get a Stripe event',
        description: 'One event with its payload (clipped), pending webhook count and the request that caused it.',
        inputSchema: { event_id: z.string().regex(/^evt_[A-Za-z0-9]+$/) },
      }, guard(({ event_id }) => getEvent(stripe, event_id)));
    },

    async status() {
      const [bal, accts, undelivered] = await Promise.all([
        stripe.balance.retrieve(),
        stripe.accounts.list({ limit: 100 }),
        stripe.events.list({ limit: 100, delivery_success: false }),
      ]);
      const avail = bal.available.map((x) => money(x.amount, x.currency)).join(', ') || '0';
      const blocked = accts.data.filter((a) => !a.payouts_enabled || !a.charges_enabled).length;
      const due = accts.data.filter((a) => (a.requirements?.currently_due?.length ?? 0) > 0).length;
      return `stripe (${mode}): available ${avail}; ${accts.data.length}${accts.has_more ? '+' : ''} connected accounts, ${blocked} blocked, ${due} with requirements due; ${undelivered.data.length}${undelivered.has_more ? '+' : ''} events with failing webhook delivery`;
    },

    async timeline(since, limit): Promise<TimelineItem[]> {
      const res = await stripe.events.list({ limit, created: { gte: Math.floor(since.getTime() / 1000) } });
      return res.data.map((e) => {
        const obj = e.data?.object as { id?: string; amount?: number; currency?: string } | undefined;
        const amt = obj?.amount != null && obj.currency ? ` ${money(obj.amount, obj.currency)}` : '';
        return { at: new Date(e.created * 1000), source: 'stripe', kind: e.type, text: `${obj?.id ?? ''}${amt}${e.account ? ` on ${e.account}` : ''}${e.pending_webhooks ? ` (webhook pending x${e.pending_webhooks})` : ''}` };
      });
    },
  };
}
