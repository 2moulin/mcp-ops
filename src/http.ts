/** A fetch that speaks JSON, carries a bearer token, and turns HTTP failures into readable errors. */
export type Fetcher = (url: string, init?: RequestInit) => Promise<Response>;

export class HttpError extends Error {
  constructor(public status: number, public body: string, url: string) {
    super(`HTTP ${status} from ${url.replace(/\?.*$/, '')}: ${body.slice(0, 300)}`);
  }
}
...
export function jsonClient(baseUrl: string, headers: Record<string, string>, fetcher: Fetcher = fetch) {
  return async <T>(path: string, query?: Record<string, string | number | undefined>): Promise<T> => {
    ...
    const res = await fetcher(url.toString(), { headers: { accept: 'application/json', ...headers } });
